...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			output.param = half4(scale, bias - outline, bias + outline, bias);
			//output.param = half4(input.normal.xyz,1);
			...
			output.underlayColor = half4(input.normal,1);
			#endif
		...
		{
			UNITY_SETUP_INSTANCE_ID(input);
			// return fixed4(input.param.x,input.param.x,input.param.x,1);
			half d = tex2D(_MainTex, input.texcoord0.xy).a * input.param.x;
			half4 c = input.faceColor * saturate(d - input.param.w);
			// return input.outlineColor;
			#ifdef OUTLINE_ON
			// half outline = input.param.y - input.param.z;
			// half outlineAlpha =  saturate((d + outline * 0.5)) * sqrt(min(1.0, outline));
			// c = lerp(input.faceColor,input.outlineColor,outlineAlpha);
			...
			//return float4(input.underlayColor.rgb * input.underlayColor.a, input.underlayColor.a);
			//return float4(input.underlayColor.a,input.underlayColor.a,input.underlayColor.a,input.underlayColor.a);
			#endif
